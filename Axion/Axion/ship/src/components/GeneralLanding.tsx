    ...
    <div className="relative h-screen w-full flex flex-col overflow-hidden bg-[#05000A] text-white select-none font-sans">
      ...
      <nav className="relative z-50 flex items-center justify-between px-12 py-8 bg-transparent">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-10 pointer-events-auto">
          {[
            { name: 'Simulations', state: 'selection' },
            { name: 'Company', state: 'company' },
            { name: 'Support', state: 'support' }
          ].map(link => (
            <button 
              key={link.name} 
              onClick={() => onEnter(link.state)} 
              className="text-[10px] font-bold tracking-[0.3em] uppercase opacity-40 hover:opacity-100 transition-all hover:scale-105 cursor-pointer bg-transparent border-none text-white outline-none"
            >
              {link.name}
            </button>
          ))}
