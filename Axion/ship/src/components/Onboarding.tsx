...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const steps = [
    ...
    },
    {
      ...
      subtitle: "What excites you most in a career?",
      options: ["Leadership", "Adventure", "Technology", "Creativity", "Helping People"],
      field: "excitement",
      icon: Target
    },
    {
      title: "Crisis Response",
      subtitle: "How do you react under extreme pressure?",
      options: ["Stay Calm", "Panic", "Think Quietly", "Take Charge"],
      field: "pressure",
      icon: Shield
    },
    {
      title: "Core Values",
      subtitle: "What matters most to your future self?",
      options: ["Money", "Freedom", "Stability", "Passion", "Travel"],
      field: "priority",
      icon: Compass
    }
  ];
  ...
  const handleNext = () => {
    if (step < steps.length - 1) {
      ...
      onComplete({
        name: formData.name,
        age: formData.age,
        traits: [formData.excitement, formData.pressure, formData.priority]
      });
    }
  };
    ...
    <div className="relative h-screen w-full flex items-center justify-center bg-[#01030a] overflow-hidden font-sans">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <h3 className="text-3xl md:text-5xl font-black text-white mb-10 tracking-tighter uppercase leading-[0.95] font-studio relative z-20">
            {currentStepData.subtitle.split(' ').map((word, i) => (
              <span key={i} className={cn(i === 2 ? "text-sky-500 neon-text" : "")}>
                {word}{' '}
              </span>
            ))}
          </h3>
      ...
      {/* Decorative Side UI - Vault Style */}
